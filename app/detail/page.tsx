import { fetchProductDetail, fetchProductRatings, fetchProductSentiment, fetchProductImages, fetchReviewList } from '@/lib/api';
...
export default async function DetailPage({ searchParams }: DetailPageProps) {
  ...
  const [initialData, initialRatingsData, initialSentimentData, initialImagesData, initialReviewsData] = await Promise.all([
    productId > 0
  ...
  return (
    <div className="w-full min-h-screen bg-neutral-100">
      ...
      <div className="w-full max-w-[1440px] mx-auto px-[100px] py-8 flex flex-col gap-8">
        <DetailContent initialData={initialData} initialRatingsData={initialRatingsData} initialSentimentData={initialSentimentData} initialImagesData={initialImagesData} initialReviewsData={initialReviewsData} />
      </div>
    </div>
  );
}
